...
#include "MotokoLanguageRuntime.h"
...
MotokoLanguageRuntime::MotokoLanguageRuntime(Process *process)
    : LanguageRuntime(process)
{
}

LanguageRuntime *
MotokoLanguageRuntime::CreateInstance(Process *process,
                                    lldb::LanguageType language) {
  if (language == eLanguageTypeMotoko)
    return new MotokoLanguageRuntime(process);
  return nullptr;
}

void MotokoLanguageRuntime::Initialize() {
  PluginManager::RegisterPlugin(GetPluginNameStatic(), "Motoko language runtime",
                                CreateInstance);
}

void MotokoLanguageRuntime::Terminate() {
  PluginManager::UnregisterPlugin(CreateInstance);
}

lldb_private::ConstString MotokoLanguageRuntime::GetPluginNameStatic() {
  static ConstString g_name("motoko");
  return g_name;
}

lldb_private::ConstString MotokoLanguageRuntime::GetPluginName() {
  return GetPluginNameStatic();
}

uint32_t MotokoLanguageRuntime::GetPluginVersion() {
  return 1;
}

bool MotokoLanguageRuntime::CouldHaveDynamicValue(ValueObject &in_value) {
  return in_value.GetCompilerType().IsPossibleDynamicType(nullptr, false, false);
}

bool MotokoLanguageRuntime::GetDynamicTypeAndAddress(
...
TypeAndOrName
MotokoLanguageRuntime::FixUpDynamicType(const TypeAndOrName &type_and_or_name,
                                      ValueObject &static_value) {
  return type_and_or_name;
}

lldb::ThreadPlanSP
MotokoLanguageRuntime::GetStepThroughTrampolinePlan(Thread &thread,
						  bool stop_others) {
