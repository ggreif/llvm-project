...
TypeSP DWARFASTParserMotoko::ParseSimpleType(const DWARFDIE &die) {
  DWARFDIE encoding_type;
  ...
  for (auto &&value : IterableDIEAttrs(die)) {
    ...
    case DW_AT_encoding:
      ...
      break;
    case DW_AT_type:
      encoding_type = value.second.Reference();
      break;
    }
  }
  ...
  case DW_TAG_base_type:
    ...
    // to treat Motoko references as CompilerType references.
  case DW_TAG_typedef:
  ...
  return TypeSP(new Type(die.GetID(), dwarf, type_name_const_str,
			 byte_size, NULL, dwarf->GetUID(encoding_type),
			 encoding_data_type, Declaration(), compiler_type,
			 resolve_state));
}

TypeSP DWARFASTParserMotoko::ParseArrayType(const DWARFDIE &die) {
  DWARFDIE type_die;

  for (auto &&value : IterableDIEAttrs(die)) {
    switch (value.first) {
    case DW_AT_type:
      type_die = value.second.Reference();
      break;
    }
  }
  ...
  TypeSP type_sp(new Type(die.GetID(), dwarf, type_name_const_str,
			  element_type->GetByteSize(), NULL, dwarf->GetUID(type_die),
...
TypeSP DWARFASTParserMotoko::ParseFunctionType(const DWARFDIE &die) {
...
TypeSP DWARFASTParserMotoko::ParseStructureType(const DWARFDIE &die) {
  ...
  if (compiler_type_was_created) {
    ...
    dwarf->m_forward_decl_clang_type_to_die[compiler_type.GetOpaqueQualType()] =
      die.GetDIERef()->die_offset();
  }

  return type_sp;
}

TypeSP DWARFASTParserMotoko::ParseCLikeEnum(const DWARFDIE &die) {
  const char *type_name_cstr = NULL;
  ConstString type_name_const_str;
  SymbolFileDWARF *dwarf = die.GetDWARF();
  ...
  // allows registering two types of the same name when reading a Motoko
  // enum.
  ...
  for (auto &&child_die : IterableDIEChildren(die)) {
	...
	/*	dwarf->GetObjectFile()->GetModule()->LogMessage(
          log, "DWARFASTParserMotoko::ParseCLikeEnum (die = 0x%8.8x) %s "
               "is invalid)",
...
TypeSP DWARFASTParserMotoko::ParseTypeFromDWARF(
  ...
  if (die) {
    ...
    /*if (log) {
      dwarf->GetObjectFile()->GetModule()->LogMessage(
          log, "DWARFASTParserMotoko::ParseTypeFromDWARF (die = 0x%8.8x) %s name = "
               "'%s')",
...
bool DWARFASTParserMotoko::CompleteTypeFromDWARF(const DWARFDIE &die,
					       lldb_private::Type *type,
					       CompilerType &compiler_type) {
  // We don't currently use type completion for Motoko.
  return bool(die);
}

Function *DWARFASTParserMotoko::ParseFunctionFromDWARF(lldb_private::CompileUnit &comp_unit,
						     const DWARFDIE &die) {
...
DWARFASTParserMotoko::GetDeclContextForUIDFromDWARF(const DWARFDIE &die) {
...
DWARFASTParserMotoko::GetDeclForUIDFromDWARF(const DWARFDIE &die) {
...
/*
std::vector<DWARFDIE>
DWARFASTParserMotoko::GetDIEForDeclContext(lldb_private::CompilerDeclContext decl_context) {
...
void DWARFASTParserMotoko::EnsureAllDIEsInDeclContextHaveBeenParsed(
    lldb_private::CompilerDeclContext decl_context) {
  /*auto opaque_decl_ctx =
      (MotokoDeclContext *)decl_context.GetOpaqueDeclContext();
