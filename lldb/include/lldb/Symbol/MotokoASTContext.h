...
#ifndef liblldb_MotokoASTContext_h_
#define liblldb_MotokoASTContext_h_
...
class MotokoASTContext : public TypeSystem {
  // LLVM RTTI support
  static char ID;

public:
  MotokoASTContext();
  ~MotokoASTContext() override;
  ...
  struct TypeNameMap {
    std::map<MotokoType *, std::string> name_map;
    unsigned counter = 0;
    // Holds the source code for the typedefs themselves.
    std::string typedefs;

    bool Tag(MotokoType *type, std::string *tagname) {
...
private:
  ...
  std::set<std::unique_ptr<MotokoType>> m_types;
  std::unique_ptr<DWARFASTParser> m_dwarf_ast_parser_ap;

  std::unique_ptr<MotokoDeclContext> m_tu_decl;

  CompilerType CacheType(MotokoType *new_type);

  MotokoASTContext(const MotokoASTContext &) = delete;
  const MotokoASTContext &operator=(const MotokoASTContext &) = delete;
};

class MotokoASTContextForExpr : public MotokoASTContext {
public:
  MotokoASTContextForExpr(lldb::TargetSP target) : m_target_wp(target) {}
  UserExpression *
...
#endif // liblldb_MotokoASTContext_h_
